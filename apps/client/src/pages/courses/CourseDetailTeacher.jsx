...
const CourseDetailTeacher = ({ user }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const canManageCourse = user.role === 'administrador' || loadedCourse.created_by === user.id;

        setCourse(loadedCourse);

        if (canManageCourse) {
          const statsRes = await axios.get(`/api/courses/${id}/stats`);
  ...
  const canManageCourse = user.role === 'administrador' || course.created_by === user.id;
  ...
  return (
    <div className="course-teacher">
      <div className="course-teacher-container">
        ...
        {course.description && (
          <p className="course-teacher-description">{course.description}</p>
        )}
